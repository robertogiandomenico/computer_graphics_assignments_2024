...
// **A10** Place here the initialization of the the DescriptorSetLayout
		DSLEarth.init(this, {
				{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_ALL_GRAPHICS, sizeof(EarthMatricesUniformBufferObject), 1},
				...
				{3, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 2, 1},  //normal map
				...
				{6, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_ALL_GRAPHICS, sizeof(EarthParamsUniformBufferObject), 1}
		});
...
// **A10** Add the commands to bind the pipeline, the mesh its two descriptor setes, and the draw call of the planet
		...
		DSGlobal.bind(commandBuffer, PEarth, 0, currentImage);	// global
		DSEarth.bind(commandBuffer, PEarth, 1, currentImage);   // Earth
...
// **A10** Add to compute the uniforms and pass them to the shaders. You need two uniforms: one for the matrices, and the other for the material parameters.
		...
		glm::mat4 normalMatrix = glm::mat4(1.0f);  // Identity matrix (normal matrix for the identity world matrix)
		glm::mat4 wvpMatrix = ViewPrj;  // World-View-Projection matrix (set to variable ViewPrj)

		EarthMatricesUniformBufferObject earthMatrices{};
		EarthParamsUniformBufferObject earthParams{};


		earthMatrices.mvpMat = wvpMatrix;
		earthMatrices.mMat = worldMatrix;
		earthMatrices.nMat = normalMatrix;

		// These informations should be used to fill the Uniform Buffer Object in Binding 0 of your DSL
		DSEarth.map(currentImage, &earthMatrices, 0);
