...
layout(binding = 2) uniform GlobalUniformBufferObject {
    ...
    if (cosAlpha > 0.0) {
        if (cosAlpha <= 0.1) {
            diffuseFactor = mix(0.0, 0.15, cosAlpha / 0.1);
        } else if (cosAlpha <= 0.7) {
            diffuseFactor = 0.15;
        } else if (cosAlpha <= 0.8) {
            diffuseFactor = mix(0.15, 1.0, (cosAlpha - 0.7) / 0.1);
        } else {
            diffuseFactor = 1.0;
        }
    }
    vec3 Diffuse = Md * diffuseFactor;
    ...
    if (cosBeta > 0.9) {
        if (cosBeta <= 0.95) {
            specularFactor = mix(0.0, 1.0, (cosBeta - 0.9) / 0.05);
        } else {
            specularFactor = 1.0;
        }
    }
    vec3 Specular = Ms * specularFactor;
